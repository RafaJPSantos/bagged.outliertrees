...
predict.bagged.outliertrees <- function(object, newdata, min_outlier_score = 0.95, nthreads = parallel::detectCores(), ...) {
  ...
  bagged_predictions <- bagged_predictions %>%
    ...
    dplyr::group_by(row_name) %>%
    suppressWarnings(dplyr::mutate(statistics.thr = ifelse(!is.na(as.numeric(statistics.thr)),
    ...
    dplyr::rowwise() %>%
    suppressWarnings(dplyr::mutate(condition.value_comp = ifelse(!is.na(as.numeric(unlist(strsplit(condition.value_comp, ", "))[1])),
